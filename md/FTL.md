...
> ●"package"、"FreeType Project"、"FreeType archive"は、"FreeType
> Project"として著作者(David Turner, Robert Wilhelm, and Werner
> Lemberg)が頒布する元々のソースコード、バイナリ、関連文書の全てをさす。
> ●サブライセンスは、本ライセンスを付与された者が、再度、第三者にその付与されたライセンスを供与することを指す。
                        ...
                        "_permission_actions": [
                            {
                                "_action_baseUri": "http://open-license.hitachi.com/",
                                "_action_schemaVersion": "0.1",
                                "_action_description": "ãµãã©ã¤ã»ã³ã¹ã¯ãå½è©²ã©ã¤ã»ã³ã¹ãä»ä¸ãããèããååº¦ãç¬¬ä¸èã«ãã®ä»ä¸ãããã©ã¤ã»ã³ã¹ãä¾ä¸ãããã¨ãæãã",
