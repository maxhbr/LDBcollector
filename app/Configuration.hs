...
ratingRules =
  let addRule desc fun = tell . (: []) $ RatingRule desc fun
      hasPossitiveJudgements l =
        let fun b j =
              b ||
              (case j of
   ...
   in execWriter $ do
        addRule "NonComercial is a no-go" $ \l ->
          case unpackRLSR (getImpliedNonCommercial l) of
            ...
            _         -> id
        addRule
          "should have at least one positive and no negative rating to be Go" $ \l ->
        ...
        addRule "Fedora bad Rating implies at least Stop" $ \l ->
          case M.lookup fedoraLFC (unpackSLSR $ getImpliedJudgement l) of
            Just (NegativeJudgement _) ->
              removeRatingFromState RGo . removeRatingFromState RAttention
            _ -> id
        ...
        addRule
          "possitive Rating by BlueOak helps, and if no other rating is negative it implies Go" $ \l ->
          case M.lookup blueOakLFC (unpackSLSR $ getImpliedJudgement l) of
            Just (PositiveJudgement _) ->
              if hasNegativeJudgements l
...
overrides
  -- Some Judgements
 =
  [ (emptyOverride "BSD-4-Clause")
      { oRatingState = Just (RatingState False True True True)
      , oJudgement =
          Just
            (NegativeJudgement
               "Advertisement clause (3.) is complicated and prone to conflicts")
      }
  , (emptyOverride "BSD-4-Clause-UC")
      { oRatingState = Just (RatingState False True True True)
      , oJudgement =
          Just
            (NegativeJudgement
  ...
  -- Compatibility
  [ (emptyOverride "GPL-2.0-only")
      { oCompatibilities =
          Just
            (isIncompatibleBothWays "Apache-2.0" <>
  ...
  map
    (\sn ->
       (emptyOverride sn)
         {oRatingState = Just (FinalRating RNoGo), oNonCommecrial = Just True})
    [ "CC-BY-NC-1.0"
    , "CC-BY-NC-2.0"
    , "CC-BY-NC-2.5"
    , "CC-BY-NC-3.0"
   ...
   ++
  map
    (\(sn, oIds) -> (emptyOverride sn) {oOtherLicenseIds = oIds})
    [ ( "GPL-1.0-only"
      , ["GPL-1.0", "GPL1.0", "GPL1", "GNU General Public License Version 1"])
    , ( "GPL-2.0-only"
      , [ "GPL-2.0"
        , "GPL2.0"
        , "GPL2"
    ...
    , ( "GPL-2.0-or-later"
      , ["GPL-2.0+", "GPL2.0+", "GPL2+", "GPL (v2 or later)"])
    , ( "GPL-3.0-or-later"
      , ["GPL-3.0+", "GPL3.0+", "GPL3+", "GPL (v3 or later)"])
    , ( "LGPL-2.1-or-later"
      , ["LGPL-2.1+", "LGPL2.1+", "LGPL2.1+", "LGPL (v2.1 or later)"])
    , ( "LGPL-3.0-or-later"
      , [ "LGPL-3.0+"
        , "LGPL-3+"
        , "LGPL3.0+"
