              ...
              <el-card style="height: 6%;">
                ...
                <div class="file-url" v-loading="loading"
                  ...
                  <el-upload class="avatar-uploader" id="uploader" ref="uploader" action="#" :show-file-list="true"
                    ...
                    <div class="el-upload__tip" slot="tip">Only <strong>zip/rar</strong> files can be uploaded. Ensure
                      that there is a file named LICENSE in the root directory of your project.</div>
                  </el-upload>
                      ...
                      <el-table-column label="license">
                        <template slot-scope="scope">
                          <el-popover placement="bottom" width="400" trigger="hover">
                            <span>{{ depend_dict[scope.row.path] || 'No conflicts' }}</span>
...
export default {
  ...
  methods: {
    ...
    generate_depend_dict() {
      var temp_depend = {};
      for (const file of Object.keys(this.check_res.licenses_in_files)) {
        temp_depend[file] = ''
      }

      console.log(temp_depend);
      for (const license of this.check_res.confilct_depend_dict) {
        temp_depend[license.src_file] += 'Incompatibile with ' + license.dest_file + ':' + license.dest_license + ';'
        temp_depend[license.dest_file] += 'Incompatibile with ' + license.src_file + ':' + license.src_license + ';'
      }
      console.log(temp_depend);
    ...
    licenses_row_class({ row, rowIndex }) {
      for (const pair of this.check_res.confilct_depend_dict) {
        if (row.path == pair.dest_file || row.path == pair.src_file) {
