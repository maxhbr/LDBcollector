...
data LicenseGraph = LicenseGraph
...
getLicenseGraphSize (LicenseGraph gr node_map node_map_rev _ _) =
  let gr_size = length $ G.nodes gr
...
insertEdge =
  let insertLEdge :: G.LEdge LicenseGraphEdge -> WithFactM ()
      insertLEdge edge = do
...
toLicenseNameGraph =
  let edgeFun :: (LicenseGraphEdge, G.Node) -> Maybe (LicenseNameRelation, G.Node)
      ...
      contextFun (incoming, n, LGName ln, outgoing) =
        let incoming' = nub $ mapMaybe edgeFun incoming
