...
instance LicenseFactC GoogleLicense where
    ...
    getImpliedStmts (GoogleLicense _ _ ty desc) = case ty of
        "restricted"        -> [LicenseRating "Google" (NegativeLicenseRating ty (Just desc))]
        "reciprocal"        -> [LicenseRating "Google" (NeutralLicenseRating ty (Just desc))]
        "notice"            -> [LicenseRating "Google" (PositiveLicenseRating ty (Just desc))]
        "permissive"        -> [LicenseRating "Google" (PositiveLicenseRating ty (Just desc))]
        "unencumbered"      -> [LicenseRating "Google" (PositiveLicenseRating ty (Just desc))]
        "by_exception_only" -> [LicenseRating "Google" (NegativeLicenseRating ty (Just desc))]
        _                   -> []
