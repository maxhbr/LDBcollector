...
data OrtLicenseCategorizationRaw = OrtLicenseCategorizationRaw
  { _id :: Text,
    _categories :: [Text]
  }
  deriving (Show, Eq, Ord, Generic)
...
data OrtLicenseClassificationFile = OrtLicenseClassificationFile
...
data OrtLicenseCategorization = OrtLicenseCategorization
...
instance LicenseFactC OrtLicenseCategorization where
  ...
  getImpliedStmts a@(OrtLicenseCategorization _ cs) =
    map
      ( \(OrtCategory name mdesc) -> case mdesc of
          Just desc -> scopedTagStmt (getType a) name desc
          _ -> LicenseTag (ScopedLicenseTag (getType a) name NoLicenseTagText)
      )
      cs

factsFromClassificationFile :: OrtLicenseClassificationFile -> [OrtLicenseCategorization]
factsFromClassificationFile (OrtLicenseClassificationFile categories categorizations) =
  let categoriesMap :: Map.Map Text OrtCategory
   ...
   in map
        ( \(OrtLicenseCategorizationRaw licenseNameString categories) ->
            OrtLicenseCategorization (newNLN "SPDX" licenseNameString) (map lookupCategory categories)
        )
        categorizations
