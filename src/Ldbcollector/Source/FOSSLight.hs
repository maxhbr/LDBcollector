...
import Database.SQLite.Simple.FromRow qualified as S
import Database.SQLite.Simple.Internal qualified as S
import Database.SQLite.Simple.Ok qualified as S
import Database.SQLite.Simple.Types qualified as S
...
data FOSSLight_License = FOSSLight_License
  { _fossLight_id :: Int, --  1    `LICENSE_ID` int(11) NOT NULL AUTO_INCREMENT COMMENT '라이선스ID',
    ...
    _fossLight_CREATOR :: (),
    -- 13    `CREATOR` varchar(50) DEFAULT NULL COMMENT '등록자',
    _fossLight_CREATED_DATE :: (),
    -- 14    `CREATED_DATE` datetime DEFAULT current_timestamp() COMMENT '등록일',
    _fossLight_MODIFIER :: (),
    -- 15    `MODIFIER` varchar(50) DEFAULT NULL COMMENT '수정자',
    _fossLight_MODIFIED_DATE :: (),
    -- 16    `MODIFIED_DATE` datetime DEFAULT current_timestamp() ON UPDATE current_timestamp() COMMENT '수정일',
...
instance S.FromRow FOSSLight_License where
  fromRow =
    let ynToBool :: Maybe String -> Bool
...
data FOSSLightFact = FOSSLightFact
...
instance LicenseFactC FOSSLightFact where
  ...
  getApplicableLNs (FOSSLightFact license nicks) =
    LN (_fossLight_name license)
      `AlternativeLNs` map LN (maybeToList (_fossLight_SHORT_IDENTIFIER license))
      `ImpreciseLNs` map LN nicks
            ...
            concat
              <$> mapM
                ( \(i, name) -> do
                    ...
                    let handleUnicodeException :: T.UnicodeException -> IO [FOSSLight_License]
