...
(deftest name->expressions-tests
  ...
  (testing "SPDX license ids"
    ...
    (is (valid= #{"GPL-2.0-with-classpath-exception"}   (name->expressions "GPL-2.0-with-classpath-exception"))))
  (testing "Public domain and proprietary/commercial"
    (is (valid= #{(public-domain)}                      (name->expressions "Public Domain")))
    (is (valid= #{(public-domain)}                      (name->expressions "Public domain")))  ; Test lower case
    (is (valid= #{(public-domain)}                      (name->expressions "              Public domain   ")))  ; Test whitespace
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Proprietary")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Commercial")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "All rights reserved"))))
  ...
  (testing "All names seen in POMs on Clojars as of 2023-07-13"
    ...
    (is (valid= #{"AGPL-3.0-only"}                      (name->expressions "GNU Affero General Public License Version 3")))
    (is (valid= #{"AGPL-3.0-only"}                      (name->expressions "GNU Affero General Public License Version 3; Other commercial licenses available.")))  ; ####TODO: THINK MORE ABOUT THIS ONE!!!
    (is (valid= #{"AGPL-3.0-only"}                      (name->expressions "GNU Affero General Public License v3")))
    (is (valid= #{"AGPL-3.0-only"}                      (name->expressions "GNU Affero General Public License v3.0")))
    (is (valid= #{"AGPL-3.0-only"}                      (name->expressions "GNU Affero General Public License, Version 3")))
    (is (valid= #{"AGPL-3.0-only"}                      (name->expressions "GNU Affero General Public License, version 3")))
    (is (valid= #{"AGPL-3.0-or-later"}                  (name->expressions "AGPL")))  ; Listed license missing version - we assume the latest
    ...
    (is (valid= #{"AGPL-3.0-or-later"}                  (name->expressions "Affero General Public License")))
    (is (valid= #{"AGPL-3.0-or-later"}                  (name->expressions "Affero General Public License,")))  ; Listed license missing version - we assume the latest
    (is (valid= #{"AGPL-3.0-or-later"}                  (name->expressions "GNU AGPL-V3 or later")))
    (is (valid= #{"AGPL-3.0-or-later"}                  (name->expressions "GNU Affero General Public Licence")))  ; Listed license missing version - we assume the latest
    (is (valid= #{"AGPL-3.0-or-later"}                  (name->expressions "GNU Affero General Public License (AGPL)")))  ; Listed license missing version - we assume the latest
    (is (valid= #{"AGPL-3.0-or-later"}                  (name->expressions "GNU Affero General Public License")))  ; Listed license missing version - we assume the latest
...
;    (is (valid= #{"BSD-3-Clause"}                       (name->expressions "https://github.com/smsharman/sxm-clojure-ms/blob/master/LICENSE")))                  ; Failing due to https://github.com/spdx/Spdx-Java-Library/issues/182
    ...
    (is (valid= #{"CC-BY-4.0"}                          (name->expressions "CC Attribution 4.0 International with exception for binary distribution")))
    ...
    (is (valid= #{"EPL-2.0 AND LGPL-3.0-or-later"}      (name->expressions "Dual: EPL and LGPL")))  ; Listed license missing version - we assume the latest
    ...
    (is (valid= #{"GPL-3.0-or-later"}                   (name->expressions "GNU General Public License,version 2.0 or (at your option) any later version")))
...
;    (is (valid= #{"MIT"}                                (name->expressions "https://github.com/clanhr/clanhr-service/blob/master/LICENSE")))  ; Failing due to https://github.com/spdx/Spdx-Java-Library/issues/182
    ...
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "All Rights Reserved")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "All rights reserved")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Copyright & all rights reserved Lean Pixel")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Copyright 2013 The Fresh Diet. All rights reserved.")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Copyright 2017 All Rights Reserved")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Not fit for public use so formally proprietary software - this is not open-source")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Private License")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Private")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Proprietary License")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Proprietary")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Proprietory. Copyright Jayaraj Poroor. All Rights Reserved.")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Tulos Commercial License")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "Wildbit Proprietary License")))
    (is (valid= #{(proprietary-or-commercial)}          (name->expressions "proprietary")))
    (is (valid= #{(public-domain)}                      (name->expressions "Public Domain")))
    (is (valid= #{(str "GPL-2.0-or-later OR " (name->unlisted "Swiss Ephemeris"))} (name->expressions "GPL v2+ or Swiss Ephemeris")))
    (is (valid= #{(str "MIT AND " (proprietary-or-commercial))} (name->expressions "Dual MIT & Proprietary")))
