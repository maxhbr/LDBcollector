...
(deftest dep->ids-tests
  ...
  (testing "Valid deps - single license"
    ...
    (is (= #{"LicenseRef-lice-comb-public-domain"} (dep->ids ['aopalliance/aopalliance {:deps/manifest :mvn :mvn/version "1.0"}])))
    ...
    (is (= #{"LicenseRef-lice-comb-public-domain"} (dep->ids ['org.tukaani/xz {:deps/manifest :mvn :mvn/version "1.9"}])))
