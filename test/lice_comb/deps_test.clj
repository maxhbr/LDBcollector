...
(deftest deps-expressions-test
  (testing "Nil and empty deps"
    ...
    (is (= {} (deps-expressions {}))))
  (testing "Single deps"
    (is (valid= #{"EPL-1.0"}    (:lice-comb/licenses (get (deps-expressions {'org.clojure/clojure {:deps/manifest :mvn :mvn/version "1.10.3"}}) 'org.clojure/clojure))))
    (is (valid= #{"EPL-1.0"}    (:lice-comb/licenses (get (deps-expressions {'com.github.athos/clj-check {:deps/manifest :deps :deps/root (str gitlib-dir "/com.github.athos/clj-check")}}) 'com.github.athos/clj-check))))    ; Note: we use this git dep, as it's used earlier in the build, so we can be sure it's been downloaded before this test is run
;    (is (= #{"Apache-2.0" "LGPL-3.0-or-later"} (:lice-comb/licenses (get (deps-expressions {'com.github.jnr/jffi$native {:deps/manifest :mvn :mvn/version "1.3.11"}}))))))    ; Blocked on https://github.com/jnr/jffi/issues/141
    (is (valid= #{"Apache-2.0"} (:lice-comb/licenses (get (deps-expressions {'com.github.jnr/jffi$native {:deps/manifest :mvn :mvn/version "1.3.11"}}) 'com.github.jnr/jffi$native))))
