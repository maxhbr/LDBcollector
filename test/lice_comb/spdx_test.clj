...
(ns lice-comb.spdx-test
  (:require [clojure.test               :refer [deftest testing is use-fixtures]]
            ...
            [lice-comb.spdx             :refer [fuzzy-match-name->license-ids uri->license-ids text->ids]]))
...
(deftest fuzzy-match-name->license-ids-tests
  ...
  (testing "Names that appear in licensey things, but aren't in the SPDX license list, and don't have identified SPDX identifiers"
    (is (= #{"NON-SPDX-Public-Domain"}           (fuzzy-match-name->license-ids "Public Domain")))
    (is (= #{"NON-SPDX-Public-Domain"}           (fuzzy-match-name->license-ids "Public domain")))))

(deftest uri->license-ids-tests
  (testing "Nil, empty or blank uri"
    (is (nil?                                 (uri->license-ids nil)))
    (is (nil?                                 (uri->license-ids "")))
    (is (nil?                                 (uri->license-ids "       ")))
    (is (nil?                                 (uri->license-ids "\n")))
    (is (nil?                                 (uri->license-ids "\t"))))
  (testing "URIs that appear verbatim in the SPDX license list"
    (is (= "Apache-2.0"                       (uri->license-ids "https://www.apache.org/licenses/LICENSE-2.0")))
    (is (= "Apache-2.0"                       (uri->license-ids "http://www.apache.org/licenses/LICENSE-2.0.html")))
    (is (= "Apache-2.0"                       (uri->license-ids "https://apache.org/licenses/LICENSE-2.0.txt")))
    (is (= "Apache-2.0"                       (uri->license-ids "               https://www.apache.org/licenses/LICENSE-2.0             ")))   ; Test whitespace
    (is (let [license-id (uri->license-ids "https://www.gnu.org/licenses/agpl.txt")]
          (or (= "AGPL-3.0"      license-id)
              (= "AGPL-3.0-only" license-id))))
    (is (= "CC-BY-SA-4.0"                     (uri->license-ids "https://creativecommons.org/licenses/by-sa/4.0/legalcode")))
    (is (= "GPL-2.0-with-classpath-exception" (uri->license-ids "https://www.gnu.org/software/classpath/license.html"))))
  (testing "URIs that appear in licensey things, but aren't in the SPDX license list"
    (is (= "Apache-2.0"                       (uri->license-ids "http://www.apache.org/licenses/LICENSE-2.0")))
    (is (= "Apache-2.0"                       (uri->license-ids "https://www.apache.org/licenses/LICENSE-2.0.txt")))))
